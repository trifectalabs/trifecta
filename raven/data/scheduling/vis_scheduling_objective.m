% Modified Objective function to evaluate scheduling a training plan
...
% Differs from scheduling_objective in function h. function h will not
% penalize paths that have a diff greater than the peak value of r (i.e.
% returns h = 0)

function fitness = vis_scheduling_objective(scheduled_training_plan, buckets)
    ...
    function h = H(scheduled_training_plan)
        ...
        for n = 1:size(scheduled_training_plan,1)-1
            ...
            effort = W(act);
            diff = abs((R(effort, (effort/200)) - R(effort,(scheduled_training_plan(n+1,3)-act(3))/96)));
            if diff > (R(effort, effort/200) - R(effort,0))
                h = 0;
            else
                h = h + diff;
            end
